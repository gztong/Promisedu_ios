...
- (IBAction)createAccount:(id)sender {
    ...
    for(id <OEXRegistrationFieldController> controller in self.fieldControllers) {
        if([controller isValidInput]) {
            if([controller hasValue]) {
                ...
                NSLog(@"%@ %@",[controller field].name, [controller currentValue] );
            }
        }
    ...
    // TONG firebase
    NSString *email1 = [parameters objectForKey:@"email"];
    NSString *password1 = [parameters objectForKey:@"password"];
    
    Firebase *ref = [[Firebase alloc] initWithUrl:@"https://promisedu.firebaseio.com"];
    
    [ref createUser:email1 password:password1
